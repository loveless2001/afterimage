(function () {
  ...
  function show(scene,choices,options={}){
    ...
    if(scene.puzzle)renderPuzzle();escapeAction=options.escape||closePanel;mobile();document.querySelector('.dialog').scrollTop=0;$('choices').querySelector('button:not(:disabled)')?.focus({preventScroll:true});
  }
  const leave=(label='Step away')=>({label,run:closePanel});
  function encounter(id){current=id;if(id==='inbox')return inbox();if(s.arrival==='received'&&id===M.arrivalStations[s.chapter])return arrivalEncounter();if(id==='handoff'&&s.chapter<4&&s.phase!=='finished')return report();const scene=Story.encounter(s,id);if(!scene){toast('Nothing to use here yet.');return;}const choices=[...(scene.choices||[])];if(pendingMilestone)choices.unshift({label:'Look at what changed',primary:true,run:closePanel});if(!choices.some(c=>c.action==='$close'))choices.push(leave());show(scene,choices);}
  ...
  function renderPuzzle(){
    ...
    add('text',{x:18,y:24},'PARCEL');add('text',{x:294,y:24},'ARRIVAL LAMP');
    add('rect',{x:19,y:74,width:18,height:18,fill:'#ece8d4'});
    for(let i=0;i<3;i++){const x=88+i*95,on=Boolean(s.circuit&(1<<i)),y=on?65:105;add('path',{d:`M ${x-40} 83 L ${x-17} 65 H ${x+17} L ${x+40} 83 M ${x-40} 83 L ${x-17} 105 H ${x+17} L ${x+40} 83`,class:'wire'});add('path',{d:`M ${x-40} 83 L ${x-17} ${y} H ${x+17} L ${x+40} 83`,class:'wire lit'});add('text',{x:x-4,y:43},['A','B','C'][i]);if(trial.target!==null&&trial.target!==undefined)add('circle',{cx:x,cy:(trial.target&(1<<i))?65:105,r:5,fill:'#f2e9cd'});add('text',{x:x-19,y:136},on?'UPPER':'LOWER');}
    add('circle',{cx:379,cy:83,r:15,class:'node'+(currentWorld.signals.claimedArrival?' live':'')});if(trial.missingContact){add('path',{d:'M 310 66 l 14 34 M 324 66 l -14 34',stroke:'#c79179','stroke-width':3});add('text',{x:220,y:154},'CONTACT ABSENT');}
  ...
  function filedReport(item){
    const record=item.report,model=Reports.build({...s,chapter:item.chapter,phase:'handoff',flags:record.flags,reportDraft:{verdict:record.verdict,attachments:record.attachments.map(a=>a.id)},reportReviewed:null,trialLog:record.trials,reportArchive:true});
