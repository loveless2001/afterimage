(function () {
  ...
  function showOutcome(){
    current='outcome';const scene=Story.outcome(s);
    show(scene,[{label:'Explore another ending',primary:true,run:()=>newRun(showOutcome)},{label:'Export this final record',run:exportSave},{label:'Read the campaign journal',run:journal},{label:'View sources and inspiration',run:sources},leave('Look at what remains')],{menu:true});
    const invitation=document.createElement('aside');invitation.className='ending-invitation';
    const title=document.createElement('strong');title.textContent=scene.replay.title;
    const text=document.createElement('p');text.textContent=scene.replay.text;invitation.append(title,text);$('lines').append(invitation);
  }
  function journal(){
