(function () {
  ...
  function operation(a) {
    ...
    if (!s.requested) return a.say('request', 'INSPECTION WINDOW', 'Ask for one bounded pause.', ['Request a lamp pause, a carrier isolation, and a delivery hold for this inspection only. Each group must acknowledge before the line opens.'], [{ label: 'Request the inspection pause', primary: true, run: () => { a.act('request'); operation(a); } }, a.leave()]);
    const next = s.mode === 'central' ? s.acks.length ? [] : ['combined'] : C.groups.filter(g => !s.acks.includes(g));
    a.say('acknowledgements', 'INSPECTION WINDOW', s.acks.length + ' of 3 groups acknowledged.', [C.modes[s.mode].detail, s.acks.length === 3 ? 'The line is ready. Open it when you are ready to commit this window.' : 'Open each available reply and record the acknowledgement.'], [...next.map(g => ({ label: 'Read ' + (g === 'combined' ? 'Counter’s combined envelope' : g + ' reply'), disabled: s.mode === 'round' && g !== C.groups[s.acks.length], run: () => a.show(Story.ack(s, g), [{ label: 'Record this acknowledgement', primary: true, run: () => { a.act('ack', g); operation(a); } }, { label: 'Back to the window', run: () => operation(a) }]) })), { label: 'Open the inspection line', primary: true, disabled: s.acks.length !== 3, run: () => { a.act('operate'); operation(a); } }, a.leave()]);
  }
  function report(a) {
    const s = a.state;
